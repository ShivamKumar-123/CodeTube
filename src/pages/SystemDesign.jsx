...
import { SystemDesignCourses } from "../assets/data.js";
...
function SystemDesign() {
  ...
  const filters = [
    { key: "all",         label: "All Courses",       emoji: "🏗️" },
    { key: "hld",         label: "High Level Design",  emoji: "🌐" },
    { key: "lld",         label: "Low Level Design",   emoji: "🔩" },
    { key: "scalability", label: "Scalability",        emoji: "📈" },
  ];

  const filteredCourses = SystemDesignCourses.filter((course) => {
    const matchesSearch =
  ...
  const floatingIcons = [
    { Icon: Database,  x: "6%",  y: "15%", delay: 0,   color: "text-red-500/20",    size: 52 },
    { Icon: Server,    x: "87%", y: "10%", delay: 0.6, color: "text-yellow-400/20", size: 44 },
    { Icon: GitMerge,  x: "82%", y: "68%", delay: 1.2, color: "text-sky-400/20",    size: 48 },
    { Icon: Network,   x: "3%",  y: "70%", delay: 1.8, color: "text-yellow-400/20", size: 40 },
    { Icon: Shield,    x: "46%", y: "90%", delay: 2.4, color: "text-red-500/20",    size: 36 },
    { Icon: HardDrive, x: "28%", y: "4%",  delay: 0.3, color: "text-sky-400/15",    size: 34 },
  ];

  const stats = [
    { value: `${SystemDesignCourses.length}+`, label: "Courses",    color: "from-red-500 to-red-400"                },
    { value: "3",                               label: "Tracks",     color: "from-yellow-400 to-yellow-300"          },
    { value: "Free",                            label: "Access",     color: "from-sky-400 to-sky-300"                },
    { value: "SD",                              label: "Focused",    color: "from-red-500 via-yellow-400 to-sky-400" },
  ];

  const filterAccent = {
    all:         { border: "border-red-500",     bg: "bg-red-500/20",     text: "text-red-400",     shadow: "shadow-red-500/20"     },
    hld:         { border: "border-sky-400",     bg: "bg-sky-400/20",     text: "text-sky-300",     shadow: "shadow-sky-400/20"     },
    lld:         { border: "border-yellow-400",  bg: "bg-yellow-400/20",  text: "text-yellow-300",  shadow: "shadow-yellow-400/20"  },
    scalability: { border: "border-orange-400",  bg: "bg-orange-400/20",  text: "text-orange-300",  shadow: "shadow-orange-400/20"  },
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-12 text-center">
        ...
        >
          <div className="relative">
            <motion.div
              className="absolute rounded-2xl border-2 border-dashed border-orange-400/40"
              style={{ inset: "-6px" }}
              animate={{ rotate: 360 }}
              transition={{ duration: 12, repeat: Infinity, ease: "linear" }}
            />
            <div className="w-20 h-20 bg-gradient-to-br from-red-500 via-yellow-400 to-sky-400 rounded-2xl flex items-center justify-center shadow-2xl shadow-orange-400/30 relative z-10">
              <Database size={40} className="text-white" />
            </div>
            {[1, 2].map((i) => (
              <motion.div
                key={i}
                className="absolute inset-0 rounded-2xl border border-orange-400/20"
        ...
        >
          Master high-level and low-level system design concepts for
          scalable and robust architectures in Hindi and English.
        </motion.p>
        ...
        >
          ...
          <div className="relative mb-4">
            ...
            <motion.input
              type="text"
              placeholder="Search system design courses..."
              value={searchQuery}
        ...
        {filteredCourses.length > 0 ? (
          ...
          >
            ...
            <motion.button
              ...
              className="px-6 py-2.5 rounded-full border-2 border-orange-400 text-orange-400 text-sm font-semibold hover:bg-orange-400/10 transition-all duration-300"
