...
import { WebDevelopmentCourses } from "../assets/data.js";
...
function WebDevelopment() {
  ...
  const filteredCourses = WebDevelopmentCourses.filter((course) => {
    const matchesSearch =
  ...
  const floatingIcons = [
    { Icon: Globe,            x: "6%",  y: "15%", delay: 0,   color: "text-red-500/20",    size: 52 },
    { Icon: Layout,           x: "87%", y: "10%", delay: 0.6, color: "text-yellow-400/20", size: 44 },
    { Icon: Server,           x: "82%", y: "68%", delay: 1.2, color: "text-sky-400/20",    size: 48 },
    { Icon: Layers,           x: "3%",  y: "70%", delay: 1.8, color: "text-yellow-400/20", size: 40 },
    { Icon: Terminal,         x: "46%", y: "90%", delay: 2.4, color: "text-red-500/20",    size: 36 },
    { Icon: MonitorSmartphone,x: "28%", y: "4%",  delay: 0.3, color: "text-sky-400/15",    size: 34 },
  ];

  const stats = [
    { value: `${WebDevelopmentCourses.length}+`, label: "Courses",    color: "from-red-500 to-red-400"                },
    { value: "3",                                 label: "Tracks",     color: "from-yellow-400 to-yellow-300"          },
    { value: "Free",                              label: "Access",     color: "from-sky-400 to-sky-300"                },
    { value: "Web",                               label: "Focused",    color: "from-red-500 via-yellow-400 to-sky-400" },
  ];

  const activeFilterAccent = {
    all:       { border: "border-red-500",    bg: "bg-red-500/20",    text: "text-red-400",    shadow: "shadow-red-500/20"    },
    frontend:  { border: "border-sky-400",    bg: "bg-sky-400/20",    text: "text-sky-300",    shadow: "shadow-sky-400/20"    },
    backend:   { border: "border-green-400",  bg: "bg-green-400/20",  text: "text-green-300",  shadow: "shadow-green-400/20"  },
    fullstack: { border: "border-yellow-400", bg: "bg-yellow-400/20", text: "text-yellow-300", shadow: "shadow-yellow-400/20" },
  };

  const current = activeFilterAccent[selectedCategory];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-12 text-center">
        ...
        >
          <div className="relative">
            <motion.div
              className="absolute rounded-2xl border-2 border-dashed border-sky-400/40"
              style={{ inset: "-6px" }}
              animate={{ rotate: 360 }}
              transition={{ duration: 12, repeat: Infinity, ease: "linear" }}
            />
            <div className="w-20 h-20 bg-gradient-to-br from-red-500 via-yellow-400 to-sky-400 rounded-2xl flex items-center justify-center shadow-2xl shadow-sky-400/30 relative z-10">
        ...
        >
          Master frontend, backend and full stack development with structured
          learning paths in Hindi and English.
        </motion.p>
        ...
        >
          ...
          <div className="relative mb-4">
            ...
            <motion.input
              type="text"
              placeholder="Search web development courses..."
              value={searchQuery}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {filters.map((filter) => {
              ...
              const a = activeFilterAccent[filter.key];
              return (
                <motion.button
                  key={filter.key}
        ...
        {selectedCategory !== "all" && (
          ...
          >
            <span className={`text-xs font-semibold px-4 py-1.5 rounded-full border ${current.border} ${current.bg} ${current.text}`}>
              Showing: {filters.find((f) => f.key === selectedCategory)?.emoji}{" "}
        ...
        {filteredCourses.length > 0 ? (
          ...
          >
            ...
            <motion.button
              ...
              className="px-6 py-2.5 rounded-full border-2 border-sky-400 text-sky-400 text-sm font-semibold hover:bg-sky-400/10 transition-all duration-300"
